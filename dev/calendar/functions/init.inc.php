...
foreach ($cal_filenames as $cal_filename) {
	...
	// Otherwise it is a local calendar.
		...
		//modifed by Kyle Mulka for Mschedule 2/3/07
		$local_cals[] = urldecode(str_replace(".ics", '', $cal_filename));
...
//	$local_cals = availableCalendars($username, $password, $local_cals);
	...
	//Awsome hack for Mschedule by Kyle Mulka 2/3/07
	foreach($local_cals as $key => $value){
		preg_match('/(.*)_(.*)_.*/', $value, $matches);
        	$local_cals[$key] = "./calendars/$term/{$matches[1]}/{$matches[2]}/$value.ics";
	}
