...
public class DivisionListPanel extends DoubleListPanel
{
    ClassesPanel classesPanel;
    Vector firstColumn;
    Vector secondColumn;

    public DivisionListPanel() 
    {
	super();
    }

    public void setDivisionList (DivisionList pDivisionList)
    {
	firstColumn = new Vector(300,100);
	secondColumn = new Vector(300,100);
	for (int i=0; i<pDivisionList.getNumDivisions(); i++)
	{
	    Division d = pDivisionList.getDivisionAt(i);
	    firstColumn.addElement(d.abbrev);
	    secondColumn.addElement(d.name);
	}
